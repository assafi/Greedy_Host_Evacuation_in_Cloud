...
public class DataCoverterImpl extends DataConverter {
	...
	@Override
	public Placement convert(TestConfiguration tConfig) throws IOException,
			DataException {
		Scanner scanner = new Scanner(tConfig.getDataFile());
		if (tConfig.pack()) {
			return extractAndPack(tConfig, scanner);
		}
		return extractPlacement(tConfig, scanner);
	}

	private Placement extractAndPack(TestConfiguration tConfig, Scanner scanner)
		...
		while (scanner.hasNext()) {
			...
			if (split_data.length != 4) {
				throw new DataException("Invalid CSV format - num columns is "
						+ split_data.length + " instead of 4");
			}
		...
		Injector inj = Guice.createInjector(new PlacementModule(hosts, vms));

		return inj.getInstance(Placement.class);
	}

	private Placement extractPlacement(TestConfiguration tConfig, Scanner scanner)
			throws DataException {
		Sequencer.INSTANCE.reset();
